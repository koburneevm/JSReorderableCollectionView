...
public protocol JSReorderableCollectionViewDelegate: class {
    func reorderableCollectionView(_ collectionView: JSReorderableCollectionView,
                                   canMoveItemAt indexPath: IndexPath) -> Bool

    func reorderableCollectionView(_ collectionView: JSReorderableCollectionView,
                                   willSnapshot cell: UICollectionViewCell, at point: CGPoint) -> UIView

    func reorderableCollectionView(_ collectionView: JSReorderableCollectionView,
                                   willAppear snapshot: UIView,
                                   source cell: UICollectionViewCell,
                                   at point: CGPoint)

    func reorderableCollectionView(_ collectionView: JSReorderableCollectionView,
                                   didScrollWithTrackingPoint point: CGPoint)
}
...
public extension JSReorderableCollectionViewDelegate {
    ...
    func reorderableCollectionView(_ collectionView: JSReorderableCollectionView,
                                   willSnapshot cell: UICollectionViewCell,
                                   at point: CGPoint) -> UIView {
...
open class JSReorderableCollectionView: UICollectionView {
    ...
    /**
         Если в методе `JSReorderableCollectionViewDelegate.canMoveItemAt` возвращать `false` для всех ячеек кроме перетаскиваемой
         и управлять порядком ячеек извне, то в этой функции нужно задать `IndexPath` ячейки в которую должен
         переместиться shapshot перетаскиваемой ячейки по завершении reordering. Иначе shapshot вернется на изначальный
         IndexPath
     */
    public func setLastIndexPath(_ path: IndexPath) {
        lastIndexPath = path
    }
    ...
    @objc private func scrollHandler() {
        ...
        reorderableDelegate?.reorderableCollectionView(self, didScrollWithTrackingPoint: point)
